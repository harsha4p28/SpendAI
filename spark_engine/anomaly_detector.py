...
from agent_engine.policy_engine import load_policy
...
def run_spend_anomaly_detection():
    ...
    # Load unified policy rules
    policy = load_policy()
    po_threshold = policy.get("mandatory_po_threshold", 5000.0)
    uncat_threshold = policy.get("high_risk_uncategorized_threshold", 10000.0)
    off_hours_cfg = policy.get("off_hours_restriction_window", {})
    start_hour = off_hours_cfg.get("start_hour", 1)
    end_hour = off_hours_cfg.get("end_hour", 4)
    off_hours_max_amount = off_hours_cfg.get("max_expense_amount", 1000.0)
    ...
    try:
        ...
        # 3. Dynamic Policy Violations Sourced from policy_rules.json
        df_flagged = df_dup.withColumn(
            "no_po_violation", F.when((F.col("amount") > po_threshold) & F.col("po_number").isNull(), True).otherwise(False)
        ).withColumn(
            "high_uncategorized_violation", F.when((F.col("amount") > uncat_threshold) & (F.col("category") == "Uncategorized"), True).otherwise(False)
        ).withColumn(
            "off_hours_violation", F.when(
                (F.hour(F.col("timestamp")).between(start_hour, end_hour)) & 
                (F.col("payment_method") == "Employee Expense Reimbursement") & 
                (F.col("amount") > off_hours_max_amount), True
            ).otherwise(False)
        )
        ...
        print("Closing SparkSession cleanly...")
        spark.stop()
