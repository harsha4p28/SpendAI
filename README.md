...
> ℹ️ **Note on Benchmark Results:** The metrics below are generated by a deterministic rule-based benchmark harness (`llm_pipeline/eval_model.py`) designed to demonstrate the evaluation workflow and performance comparison interface. To produce non-simulated metrics from a trained neural network checkpoint, run `llm_pipeline/train_qlora_colab.py` on a GPU-enabled environment (such as Google Colab T4).
